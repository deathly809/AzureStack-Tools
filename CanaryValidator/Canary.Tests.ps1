...
while ($runCount -le $NumberOfIterations) {
    ...
    Add-Usecase -Name "InitializeAdminSession" -Description "Create the admin environment, login, and select subscription" -UsecaseBlock `
    {
        ...
        Invoke-Usecase -Name 'GetAzureStackAdminEndpoints' -Description "Get Endpoints from admin ARM endpoint $AdminArmEndpoint" -UsecaseBlock `
        {
            ...
            Set-Variable -Scope 1 -Name "asEndpoints" -Value $asEndpoints -PassThru
        }
                ...
                Invoke-Usecase -Name 'UploadLinuxImageToPIR' -Description "Uploads Linux image to the PIR" -UsecaseBlock `
                {
                    try {
                        if (-not (Get-AzureRmVMImage -Location $ResourceLocation -PublisherName $linuxImagePublisher -Offer $linuxImageOffer -Sku $LinuxOSSku -ErrorAction SilentlyContinue)) {
                            ...
                            Set-Variable -Scope 1 -Name "linuxUpload" -Value $true
                        }    
                    }
                    catch {
                        Remove-Item -Path $CanaryCustomImageFolder -Force -Recurse
    ...
    if (($TenantAdminCredentials) -or ($ListAvailable)) {
        Add-Usecase -Name "ValidateAdminOperationsThroughTenant" -Description "Test that Admin operations are visible from Tenant" -Prereqs "InitializeAdminSession" -UsecaseBlock `
        {
            ...
            {   
                ...
                NewAzureStackToken -AADTenantId $TenantID -EnvironmentDomainFQDN $EnvironmentDomainFQDN -Credentials $ServiceAdminCredentials -ArmEndPoint $AdminArmEndpoint
                $defaultSubscription = Get-AzureRmSubscription -SubscriptionName "Default Provider Subscription" -ErrorAction Stop
    ...
    Add-Usecase -Name 'QueryImagesFromPIR' -Description "Queries the images in Platform Image Repository to retrieve the OS Version to deploy on the VMs" -Prereqs "UploadLinuxImage" -UsecaseBlock `
    {
        ...
        Set-Variable -Scope 1 -Name "osVersion" -Value $osVersion
    ...
    Add-Usecase -Name 'QueryDeployedVMs' -Description "Queries for the VMs that were deployed using the ARM template" -Prereqs "ArmTemplates" -UsecaseBlock `
    {
        ...
        foreach ($vm in $vmList) {
            ...
            $windowsVMList += $vmObject 
        }
        Set-Variable -scope 1 -Name "canaryWindowsVMList" -Value $windowsVMList
    }
        ...
        if ($canaryWindowsVMList) {   
            ...
            foreach ($vm in $canaryWindowsVMList) {
                if ($vm.VMPublicIP -and $vm.VMName.EndsWith("VM1", 'CurrentCultureIgnoreCase')) {
                    Set-Variable -Scope 1 -Name "publicVMName" -Value $vm.VMName
                    Set-Variable -Scope 1 -Name "publicVMIP" -Value $vm.VMPublicIP[0]
                }
                if ((-not ($vm.VMPublicIP)) -and ($vm.VMPrivateIP) -and ($vm.VMName.EndsWith("VM2", 'CurrentCultureIgnoreCase'))) {
                    Set-Variable -Scope 1 -Name "privateVMName" -Value $vm.VMName
                    Set-Variable -Scope 1 -Name "privateVMIP" -Value $vm.VMPrivateIP[0]
                }
            }        
        }
    }
